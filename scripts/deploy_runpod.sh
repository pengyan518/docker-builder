...
test_image_locally() {
	...
	# docker run -d --name runpod-test --gpus all -p 8188:8188 enhou/runpod-comfyui-serverless-upscale-flux-full:latest python /comfyui/main.py --listen 0.0.0.0 --port 8188
	docker run -d \
		--name runpod-test \
		--gpus all \
		-p 8188:8188 \
